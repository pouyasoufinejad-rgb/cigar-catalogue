...
export function countryLabel(country) {
  ...
  if (/^dominican\s+republic$/i.test(value)) return 'DR';
  return value.replace(/\bDominican\s+Republic\b/gi, 'DR');
}
  ...
  return String(html || '').replace(allCardsRx, matchedCard => {
    ...
    if (Object.prototype.hasOwnProperty.call(override, 'country')) {
      card = card.replace(/<(span|div)\b([^>]*\bclass=[\"'][^\"']*\bcountry-name\b[^\"']*[\"'][^>]*)>[\s\S]*?<\/\1>/i,
        (_all, tagName, attrs) => `<${tagName}${attrs}>${esc(countryLabel(text(override.country)))}</${tagName}>`);
    }
...
export function injectRuntimeBootstrap(html) {
  ...
  const script = '<script type="module" src="/catalogue-runtime.mjs?v=144"></script>';
