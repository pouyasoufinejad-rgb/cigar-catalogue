...
test('browser runtime bootstrap is separate from the pure Value module', async () => {
  ...
  assert.match(runtimeSource, /import\(['"]\.\/catalogue-direct-edit\.mjs\?v=variant-edit-1['"]\)/, 'direct editor must own Edit catalogue');
...
test('Worker HTML transform injects the runtime bootstrap exactly once', async () => {
  ...
  assert.match(once, /<script type="module" src="\/catalogue-runtime\.mjs\?v=168"><\/script><\/body>/);
