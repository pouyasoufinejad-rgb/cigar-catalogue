...
test('standard desktop catalogue keeps three columns and overhangs to the right only', () => {
  ...
  assert.match(layoutSource, /--card-bleed:min\(120px, max\(0px, \(100vw - 100%\) \/ 2 - 16px\)\)/);
  assert.match(layoutSource, /width:calc\(100% \+ var\(--card-bleed\)\)!important/);
  assert.match(layoutSource, /margin-left:0!important/);
  assert.match(layoutSource, /margin-right:calc\(-1 \* var\(--card-bleed\)\)!important/);
  assert.doesNotMatch(layoutSource, /margin-inline:-\d+px!important/, 'no left overhang anywhere');
});
...
test('desktop header and catalogue chrome use the same horizontal overhang as the cards', () => {
  ...
  assert.match(layoutSource, /@media\(min-width:901px\)[\s\S]*\.wrap > header,[\s\S]*width:calc\(100% \+ var\(--card-bleed\)\)!important;[\s\S]*margin-left:0!important/);
...
test('large desktop catalogue and chrome widen together when the fixed sidebar is present', () => {
  assert.match(layoutSource, /@media\(min-width:1660px\)[\s\S]*html body \.grid,[\s\S]*\.wrap > header,[\s\S]*--card-bleed:min\(180px/);
});
