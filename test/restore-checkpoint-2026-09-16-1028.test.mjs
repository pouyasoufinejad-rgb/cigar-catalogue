...
import { buildCheckpointState, TARGET_COMMIT, POST_TARGET_KEY } from '../scripts/restore-checkpoint-2026-09-16-1028.mjs';

const requests = [
  {
    operation:'upsert-entry', key:'la-flor-dominicana-la-nox-petit',
    entry:{ brand:'La Flor Dominicana', title:'La Nox Petit — Single', catalogueType:'main', ring:40, productionLines:['Handmade','Wrapper: Brazilian Maduro'], practicalLines:['Single cigar','5″ × 40 Petit'] }
  },
  {
    operation:'upsert-entry', key:'my-father-la-gran-oferta-lancero',
    entry:{ brand:'My Father', title:'La Gran Oferta Lancero — Box of 20', catalogueType:'main', ring:38, productionLines:['Handmade','Wrapper: Habano Rosado'], practicalLines:['Box of 20 cigars','7.5″ × 38 Lancero'] }
  }
];
...
test('restore removes post-checkpoint catalogue and sidebar state', () => {
  ...
  const restored = buildCheckpointState({ current, targetRequests:[], targetHtml:html });
  ...
  assert.deepEqual(restored.sections.recommendationSubsections[0].entryKeys, ['static-one']);
...
test('target morning additions are restored from their publication requests and rejoin main recommendation ranking', () => {
  ...
  assert.deepEqual(restored.sections.recommendationSubsections[0].entryKeys, ['my-father-la-gran-oferta-lancero','la-flor-dominicana-la-nox-petit','older']);
  assert.equal(restored.cards['my-father-la-gran-oferta-lancero'].rank, 1);
  assert.equal(restored.cards['la-flor-dominicana-la-nox-petit'].rank, 2);
});
